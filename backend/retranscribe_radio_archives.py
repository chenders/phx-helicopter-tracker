...
def check_existing_transcriptions(
    mp3_files: List[Path],
...
def transcribe_file(mp3_file: Path, model, model_name: str = "base") -> Dict[str, Any]:
    ...
    with open(txt_file, "w") as f:
        ...
        f.write(
            f"Transcription time: {transcription_data['model_performance']['transcription_time_seconds']}s\\n"
        )
        f.write(
            f"File size: {transcription_data['model_performance']['file_size_mb']}MB\\n"
        )
        f.write("=" * 80 + "\\n\\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Re-transcribe radio archives sequentially"
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    if args.force:
        ...
        files_to_process, files_with_transcriptions = check_existing_transcriptions(
            mp3_files
        )
    ...
    for idx, mp3_file in enumerate(files_to_process, 1):
        try:
            logger.info(
                f"Processing file {idx}/{len(files_to_process)}: {mp3_file.name}"
            )
            ...
            logger.info(
                f"Progress: {progress_percent:.1f}% ({idx}/{len(files_to_process)})"
            )
    ...
    logger.info(
        f"Success rate: {(transcribed_count / len(files_to_process) * 100):.1f}%"
        if files_to_process
        else "N/A"
    )
