    ...
    if len(positions) > window_size * 2:
        for i in range(window_size, len(positions) - window_size):
            ...
            bearing_before = calculate_bearing(
                positions[i - window_size][0],
                positions[i - window_size][1],
                positions[i][0],
                positions[i][1],
            )
...
def calculate_position_confidence(
    positions: List[Dict], window_size: int = 20, turn_threshold: float = 45.0
) -> List[float]:
    """
    ...
    for i in range(len(positions)):
        ...
        for j in range(start_idx + 1, end_idx - 1):
            if j > 0 and j < len(positions) - 1:
                bearing_before = calculate_bearing(
                    positions[j - 1]["latitude"],
                    positions[j - 1]["longitude"],
        ...
        avg_angle_change = (
            total_angle_change / (window_positions - 1) if window_positions > 1 else 0
        )
    ...
    for i in range(len(confidence_scores)):
        ...
        smoothed_scores.append(
            sum(window_scores) / len(window_scores) if window_scores else 0
        )

    return smoothed_scores


def identify_sky_art_segments(
    positions: List[Dict],
    window_size: int = 100,
    ...
    for i in range(
        0, len(positions) - window_size, 25
    ):  # Step by 25 for overlapping windows
        ...
        is_sky_art = (
            lat_range > 0.0001
            and lat_range < 0.05
            and lon_range > 0.0001  # ~11m to 5.5km geographic extent
            and lon_range < 0.05
            and efficiency < 0.5
            and significant_changes > 5  # Indirect path (relaxed)
            and total_distance  # Some direction changes (relaxed)
            > 500  # At least 500m of movement (relaxed)
        )
    ...
    for seg in sky_art_segments:
        ...
        for existing in merged_segments:
            # Check for overlap
            if (
                seg["start_index"] <= existing["end_index"]
                and seg["end_index"] >= existing["start_index"]
            ):
...
def detect_hovering_patterns(positions: List[Dict]) -> List[Dict[str, Any]]:
    ...
    for i, pos in enumerate(positions):
        ...
        distance_meters = haversine_distance(
            pos.get("latitude", 0), pos.get("longitude", 0), avg_lat, avg_lon
        )
...
def detect_abnormal_flight_patterns(
    self,
    flight_id: Optional[str] = None,
    batch_size: int = 50,
    min_complexity_threshold: float = 2.0,
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if flight_id:
            ...
            query = (
                db.query(FlightLog)
                .filter(~FlightLog.id.in_(subquery))
                .order_by(FlightLog.departure_time.desc())
        ...
        total_unanalyzed = (
            db.query(FlightLog)
            .filter(~FlightLog.id.in_(db.query(AbnormalPattern.flight_log_id)))
            .count()
        )

        logger.info(
            f"Analyzing {len(flights)} flights for abnormal patterns ({total_unanalyzed} total unanalyzed)"
        )
        ...
        for flight in flights:
            ...
            if total_hover_time > 10:  # More than 10 minutes hovering
                is_abnormal = True
                abnormality_reasons.append(
                    f"Excessive hovering: {total_hover_time:.1f} minutes"
                )
            ...
            if circling_metrics["circles_detected"] > 5:
                is_abnormal = True
                abnormality_reasons.append(
                    f"Repetitive circling: {circling_metrics['circles_detected']} circles"
                )
            ...
            turn_rate = (
                complexity_metrics["sharp_turns"] / flight.flight_duration_minutes
                if flight.flight_duration_minutes and flight.flight_duration_minutes > 0
                else 0
            )
            ...
            if complexity_metrics["sharp_turns"] > 200:
                position_confidence = calculate_position_confidence(
                    pos_dicts, window_size=20, turn_threshold=45.0
                )
                max_position_confidence = (
                    max(position_confidence) if position_confidence else 0
                )
            else:
                position_confidence = []
            ...
            is_likely_sky_art = (
                complexity_metrics["sharp_turns"] > 500
                and complexity_metrics["efficiency"] < 0.001  # Much higher threshold
                and turn_rate >= 7  # Very inefficient path
                and turn_rate <= 12
                and complexity_metrics["reversals"]  # Narrower range around ALEX's 8.8
                > 100
                and flight.flight_duration_minutes  # More reversals required
                and flight.flight_duration_minutes > 80
                and max_position_confidence  # Longer flights
                > 0.1  # Require significant confidence peaks
            )

            if is_likely_sky_art:
                is_abnormal = True
                abnormality_reasons.append(
                    f"Sky art pattern detected (deliberate artistic pattern, {turn_rate:.1f} turns/min, {max_position_confidence*100:.0f}% peak confidence)"
                )
                ...
                sky_art_segments = identify_sky_art_segments(
                    pos_dicts,
                    window_size=20,
                    turn_threshold=45.0,
            ...
            # Characteristics: Extremely high turn rate, searching/tracking behavior
                ...
                complexity_metrics["sharp_turns"] > 200
                and complexity_metrics["efficiency"] < 0.05
                and turn_rate > 18
                and flight.flight_duration_minutes  # Surveillance: >18 turns/minute (typically 20-30)
                and flight.flight_duration_minutes > 60
            ):
                is_abnormal = True
                abnormality_reasons.append(
                    f"Intensive surveillance pattern detected ({turn_rate:.1f} turns/min, {complexity_metrics['sharp_turns']} total turns)"
                )
                ...
                position_confidence = calculate_position_confidence(
                    ...
                    turn_threshold=30.0,
                )  # Lower threshold for surveillance
            ...
            if is_abnormal:
                ...
                if is_likely_sky_art:
                    ...
                    pattern_type = (
                        "intensive_surveillance"  # High-frequency search patterns
                    )
            ...
            metadata = {
                "reasons": abnormality_reasons
                if is_abnormal
                else ["Normal flight pattern"],
            ...
            if pattern_type == "sky_art" and sky_art_segments:
                ...
                metadata["total_sky_art_positions"] = sum(
                    seg["length"] for seg in sky_art_segments
                )
            ...
            if position_confidence:
                ...
                metadata["avg_confidence"] = (
                    sum(position_confidence) / len(position_confidence)
                    if position_confidence
                    else 0
                )
                ...
                metadata["high_confidence_positions"] = sum(
                    1 for c in position_confidence if c > 0.7
                )
            ...
            abnormal_pattern = AbnormalPattern(
                ...
                confidence_score=min(
                    complexity_metrics["complexity"] / min_complexity_threshold, 1.0
                )
                if is_abnormal
                else 0.0,
        ...
        results["summary"] = {
            ...
            "detection_rate": len(results["abnormal_patterns_found"])
            / results["flights_analyzed"]
            if results["flights_analyzed"] > 0
            else 0,
            "most_common_abnormality": max(
                [
                    reason
                    for flight in results["abnormal_patterns_found"]
                    for reason in flight.get("abnormality_reasons", [])
                ],
                default="None",
            )
            if results["abnormal_patterns_found"]
            else "None",
            "total_unanalyzed_remaining": total_unanalyzed
            - results["flights_analyzed"],
            "batch_size": batch_size,
        }

        logger.info(
            f"Abnormal pattern detection complete: {len(results['abnormal_patterns_found'])} patterns found"
        )
...
def analyze_alex_incident(self) -> Dict[str, Any]:
    ...
    try:
        ...
        start_date = datetime(
            2025, 7, 10, 20, 0, 0, tzinfo=timezone.utc
        )  # 8 PM UTC (1 PM Phoenix)
        end_date = datetime(
            2025, 7, 11, 10, 0, 0, tzinfo=timezone.utc
        )  # 10 AM UTC (3 AM Phoenix)
        ...
        for flight in flights:
            ...
            if result.get("abnormal_patterns_found"):
                ...
                if (
                    pattern["complexity_metrics"]["sharp_turns"] > 15
                    and pattern["complexity_metrics"]["efficiency"] < 0.25
                ):
                    ...
                    logger.info(
                        f"Potential ALEX incident match found: {flight.flight_id}"
                    )
        ...
        if not results["incident_found"]:
            ...
            results[
                "note"
            ] = "Pattern not found - may need to download July 2025 flight data"
