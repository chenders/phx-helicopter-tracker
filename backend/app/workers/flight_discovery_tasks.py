...
async def _discover_flights_async(registration: str, max_pages: int, start_date: str = None, end_date: str = None) -> Dict[str, Any]:
    """
            ...
            while page <= max_pages:
                try:
                    ...
                    if start_date and end_date:
                        ...
                        data = await fr24_api_service.get_flight_summary(registration, page)
                    flights = data.get('data', [])
                    
                    if not flights:
                        logger.info(f"No more flights found for {registration} at page {page}")
                        break
                    ...
                    for flight_data in flights:
                        ...
                        logger.info(f"Discovered new flight: {fr24_id} ({registration})")
                    ...
                    if len(flights) < 100:
                        logger.info(f"Last page reached for {registration} (only {len(flights)} flights)")
                        break
                    ...
                    logger.error(f"Error processing page {page} for {registration}: {e}")
    ...
    logger.info(f"Discovery complete for {registration}: {results['flights_new']} new, "
...
def discover_all_phoenix_pd_flights(self) -> Dict[str, Any]:
    ...
    for registration in PHOENIX_PD_REGISTRATIONS:
        try:
            ...
            results["total_flights_discovered"] += discovery_result.get("flights_discovered", 0)
    ...
    logger.info(f"Discovery complete for all Phoenix PD: "
...
def discover_full_year_for_registration(self, registration: str, days_back: int = 365) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.info(f"Starting full year discovery for {registration}, going back {days_back} days")
        ...
        while current_end > end_date - timedelta(days=days_back):
            ...
            try:
                ...
                results["total_flights_discovered"] += chunk_result.get("flights_discovered", 0)
                ...
                logger.error(f"Failed to process chunk {current_start.date()} to {current_end.date()}: {e}")
                results["errors"].append(f"Chunk {current_start.date()} to {current_end.date()}: {str(e)}")
            
            # Move to next chunk
            current_end = current_start
        
        logger.info(f"Full year discovery complete for {registration}: "
...
def discover_full_year_all_phoenix_pd(self, days_back: int = 365) -> Dict[str, Any]:
    ...
    for registration in PHOENIX_PD_REGISTRATIONS:
        try:
            ...
            results["registrations_processed"].append(registration)
            results["total_flights_discovered"] += discovery_result.get("total_flights_discovered", 0)
    ...
    logger.info(f"Full year discovery complete for all Phoenix PD: "
...
def download_tracks_for_discovered_flights(self, batch_size: int = 5) -> Dict[str, Any]:
    ...
    try:
        ...
        asyncio.set_event_loop(loop)
        result = loop.run_until_complete(
            _download_tracks_async(batch_size)
        )
        loop.close()
...
async def _download_tracks_async(batch_size: int) -> Dict[str, Any]:
    ...
    try:
        ...
        unprocessed_flights = db.query(FlightDiscovery).filter(
        ...
        async with fr24_api_service:
            for flight in unprocessed_flights:
                try:
                    logger.info(f"Downloading track for {flight.fr24_id} ({flight.registration})")
                    ...
                    if not aircraft:
                        ...
                        aircraft = aircraft_crud.create(
                            ...
                            obj_in=AircraftCreate(
                                ...
                                is_phoenix_pd=flight.registration in PHOENIX_PD_REGISTRATIONS,
                                operator="Phoenix Police Department" if flight.registration in PHOENIX_PD_REGISTRATIONS else None,
                    ...
                    logger.info(f"Successfully downloaded {len(positions)} positions for {flight.fr24_id}")
    ...
    logger.info(f"Track download complete: {results['tracks_downloaded']} tracks, "
