...
def task_prerun_handler(
    ...
    try:
        ...
        try:
            ...
            important_tasks = [
                "monitor_and_download_complete_flights",
        ...
        try:
            ...
            if task_history:
                ...
                if task_history.started_at:
                    runtime = (
                        task_history.completed_at - task_history.started_at
                    ).total_seconds()
                ...
                if isinstance(result, dict):
                    ...
                    task_history.records_processed = result.get(
                        "records_processed"
                    ) or result.get("flights_processed")
            else:
            ...
            if task_history and (
                (task_history.credits_used and task_history.credits_used > 100)
                or (
                    task_history.records_processed
                    and task_history.records_processed > 100
                )
                or (task_history.runtime_seconds and task_history.runtime_seconds > 60)
            ):
                task_event = TaskEvent(
                    task_name=sender.name,
            ...
            update_task_metrics(
                ...
                sender.name,
                success=True,
                runtime=task_history.runtime_seconds,
...
def task_failure_handler(
    ...
    try:
        ...
        try:
            ...
            db.commit()
            logger.warning(
                f"Task {task_id} ({sender.name if sender else 'unknown'}) failed: {exception}"
            )
        finally:
            db.close()
    except Exception as e:
