...
async def resolve_issue(
    issue_id: int, resolution_notes: Optional[str] = None, db: Session = Depends(get_db)
):
    """
...
async def reopen_issue(issue_id: int, db: Session = Depends(get_db)):
    """
...
async def get_issues_by_component(
    resolved: bool = False, db: Session = Depends(get_db)
):
    """
    Get issue counts grouped by component
    """
    issues = db.query(SystemIssue).filter(SystemIssue.is_resolved == resolved).all()
...
async def get_system_stats(db: Session = Depends(get_db)):
    ...
    stats = {
        ...
        "active_issues": db.query(SystemIssue)
        .filter(SystemIssue.is_resolved == False)
        .count(),
        "resolved_issues": db.query(SystemIssue)
        .filter(SystemIssue.is_resolved == True)
        .count(),
        "new_in_last_24h": db.query(SystemIssue)
        .filter(SystemIssue.first_seen >= last_24h)
        .count(),
        "resolved_in_last_24h": db.query(SystemIssue)
        .filter(
            and_(SystemIssue.is_resolved == True, SystemIssue.resolved_at >= last_24h)
        )
        .count(),
        "by_priority": {
            "critical": db.query(SystemIssue)
            .filter(
                and_(
                    SystemIssue.is_resolved == False, SystemIssue.priority == "critical"
                )
            )
            .count(),
            "high": db.query(SystemIssue)
            .filter(
                and_(SystemIssue.is_resolved == False, SystemIssue.priority == "high")
            )
            .count(),
            "medium": db.query(SystemIssue)
            .filter(
                and_(SystemIssue.is_resolved == False, SystemIssue.priority == "medium")
            )
            .count(),
            "low": db.query(SystemIssue)
            .filter(
                and_(SystemIssue.is_resolved == False, SystemIssue.priority == "low")
            )
            .count(),
        },
    }
