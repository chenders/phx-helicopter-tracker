...
async def get_live_tracking_from_database(
    *,
    ...
    minutes_back: int = Query(
        15, description="How many minutes of recent data to include"
    ),
    ...
    query = (
        db.query(FlightPosition, Aircraft, FlightLog)
        .join(Aircraft, FlightPosition.aircraft_id == Aircraft.id)
        .join(FlightLog, FlightPosition.flight_log_id == FlightLog.id)
        .filter(FlightPosition.timestamp >= cutoff_time)
    )
    ...
    for reg, (position, aircraft, flight_log) in aircraft_positions.items():
        ...
        is_surveillance = (
            is_hovering
            or is_circling
            or (position.altitude_feet and position.altitude_feet < 1500)
            or (
                flight_log.surveillance_likelihood
                and flight_log.surveillance_likelihood > 0.5
            )
        )
...
async def get_live_tracking_stats(db: Session = Depends(get_db)) -> Dict[str, Any]:
    ...
    last_5_min = (
        ...
        .filter(FlightPosition.timestamp >= now - timedelta(minutes=5))
        .count()
    )

    last_15_min = (
        db.query(FlightPosition)
        .filter(FlightPosition.timestamp >= now - timedelta(minutes=15))
        .count()
    )

    last_hour = (
        db.query(FlightPosition)
        .filter(FlightPosition.timestamp >= now - timedelta(hours=1))
        .count()
    )

    # Active aircraft
    active_aircraft = (
        db.query(func.count(func.distinct(FlightPosition.aircraft_id)))
        .filter(FlightPosition.timestamp >= now - timedelta(minutes=10))
        .scalar()
    )
    ...
    most_recent = (
        db.query(FlightPosition).order_by(desc(FlightPosition.timestamp)).first()
    )

    # Today's flights
    today_start = now.replace(hour=0, minute=0, second=0, microsecond=0)
    todays_flights = (
        db.query(FlightLog).filter(FlightLog.departure_time >= today_start).count()
    )
    ...
    return {
        ...
        "active_aircraft": {"total": active_aircraft, "phoenix_pd": phoenix_pd_active},
        "most_recent_update": most_recent.timestamp.isoformat()
        if most_recent
        else None,
        "minutes_since_update": round(
            (now - most_recent.timestamp).total_seconds() / 60, 1
        )
        if most_recent
        else None,
...
async def force_api_update(
    ...
    registration: Optional[str] = Query(
        None, description="Specific aircraft to update"
    ),
) -> Dict[str, Any]:
    """
