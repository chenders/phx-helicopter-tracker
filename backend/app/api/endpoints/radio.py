...
RADIO_DATA_PATH = (
    Path("/app/data/radio/phoenix_pd")
...
async def get_radio_archives(
    ...
    try:
        ...
        all_mp3_files = sorted(
            RADIO_DATA_PATH.glob("*.mp3"), key=lambda x: x.stat().st_mtime, reverse=True
        )

        # Query database for all transcriptions to create lookup map
        db_archives = (
            db.query(RadioArchive).filter(RadioArchive.transcribed == True).all()
        )
        ...
        for mp3_file in mp3_files[offset : offset + limit]:
                ...
                try:
                    async with aiofiles.open(json_file, "r") as f:
                        ...
                        if "model_performance" in trans_data:
                            archive_info["transcription_time"] = trans_data[
                                "model_performance"
                            ].get("transcription_time_seconds")
                except Exception:
                    pass
            # Otherwise, if in database, add metadata from there
                ...
                if db_archive.transcription:
                    archive_info[
                        "transcription_model"
                    ] = db_archive.transcription.model_name
                    archive_info["transcribed_at"] = (
                        db_archive.transcription.created_at.isoformat()
                        if db_archive.transcription.created_at
                        else None
                    )
...
async def get_archive_stats() -> Dict[str, Any]:
    ...
    try:
        ...
        total_size_mb = (
            sum(f.stat().st_size for f in mp3_files) / (1024 * 1024) if mp3_files else 0
        )
        ...
        if mp3_files and date_range:
            ...
            if recent_files:
                recent_size_mb = sum(f.stat().st_size for f in recent_files) / (
                    1024 * 1024
                )
                days_of_data = min(
                    7, (datetime.now() - datetime.fromtimestamp(oldest)).days
                )
                if days_of_data > 0:
            ...
            try:
                async with aiofiles.open(json_file, "r") as f:
                    ...
                    if "model_performance" in data:
                        total_transcription_time += data["model_performance"].get(
        ...
        return {
            ...
            "transcription_percentage": round(len(json_files) / len(mp3_files) * 100, 1)
            if mp3_files
            else 0,
            ...
            "average_transcription_time_seconds": round(
                total_transcription_time / len(json_files), 2
            )
            if json_files
            else 0,
        }
...
async def get_transcription(
    ...
    try:
        ...
        db_archive = (
            db.query(RadioArchive).filter(RadioArchive.filename == filename).first()
        )
        if db_archive and db_archive.transcription:
            ...
            segments = (
                ...
                .filter(RadioSegment.transcription_id == trans.id)
                .order_by(RadioSegment.start_time)
                .all()
            )
            ...
            return {
                ...
                "transcribed_at": trans.created_at.isoformat()
                if trans.created_at
                else None,
                "segments": [
                    {
...
async def trigger_archive_download(
    ...
    task = download_broadcastify_archives.delay(
        max_downloads=max_downloads, days_back=days_back
    )

    return {
        "task_id": task.id,
        "status": "Task queued",
        "parameters": {"days_back": days_back, "max_downloads": max_downloads},
    }
...
async def trigger_transcription(
    batch_size: int = Query(
        1, ge=1, le=10, description="Number of files to transcribe"
    ),
    ...
    """
    from app.workers.radio_tasks_faster_whisper import (
    ...
    try:
        ...
        archives = (
            ...
            .filter(
                and_(
                    ...
                    RadioArchive.transcribed
                    == True,  # Only return transcribed archives
                )
            )
            .order_by(RadioArchive.recording_start)
            .all()
        )
        ...
        for archive in archives:
            archive_data = {
                ...
                "recording_start": archive.recording_start.isoformat()
                if archive.recording_start
                else None,
                "recording_end": archive.recording_end.isoformat()
                if archive.recording_end
                else None,
                "duration_seconds": archive.duration_seconds,
                "has_transcription": archive.transcribed,
                "file_size_mb": round(archive.file_size_bytes / (1024 * 1024), 2)
                if archive.file_size_bytes
                else None,
            }
            ...
            if archive.transcription:
                archive_data["transcription"] = {
                    ...
                    "full_text_preview": archive.transcription.full_text[:200] + "..."
                    if len(archive.transcription.full_text) > 200
                    else archive.transcription.full_text,
...
async def get_radio_activity_by_timerange(
    ...
    include_locations: bool = Query(
        True, description="Only include segments with location data"
    ),
    db: Session = Depends(get_db),
) -> List[Dict[str, Any]]:
    """
