...
def get_pattern_analysis(
    ...
    surveillance_flights = sum(1 for f in flights if f.flight_duration_minutes and f.flight_duration_minutes > 30)
    surveillance_ratio = surveillance_flights / total_flights if total_flights > 0 else 0
    
    # Calculate hover duration (flights that stayed in one area)
    hover_events = [f for f in flights if f.flight_duration_minutes and f.flight_duration_minutes > 60]
    avg_hover_duration = sum(f.flight_duration_minutes for f in hover_events) / len(hover_events) if hover_events else 0
    ...
    for i in range(30):
        ...
        daily_flights = [f for f in flights if f.departure_time and f.departure_time.date() == (now - timedelta(days=i)).date()]
        surveillance_count = sum(1 for f in daily_flights if f.flight_duration_minutes and f.flight_duration_minutes > 30)
        violations = sum(1 for f in daily_flights if f.flight_duration_minutes and f.flight_duration_minutes > 120)
    ...
    for hour in range(24):
        hour_flights = [f for f in flights if f.departure_time and f.departure_time.hour == hour]
    ...
    neighborhoods = ["Maryvale", "South Phoenix", "Central Phoenix", "North Phoenix", "Ahwatukee"]
    ...
    discriminatory_ratio = 1.8  # Placeholder - would calculate from actual geographic data
    excessive_hovering_events = sum(1 for f in flights if f.flight_duration_minutes and f.flight_duration_minutes > 90)
