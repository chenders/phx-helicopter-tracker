...
class FR24RateLimiter:
    ...
    def __init__(self):
        ...
        # Rate limits based on FR24 subscription (STRICT 30/minute limit)
        self.limits = {
            ...
            "monthly_credits": int(
                getattr(settings, "FR24_MONTHLY_CREDIT_LIMIT", 666000)
            ),
        }
    ...
    def can_make_request(self) -> tuple[bool, str]:
        ...
        if monthly_usage >= self.limits["monthly_credits"]:
            return (
                False,
                f"Monthly credit limit reached ({monthly_usage}/{self.limits['monthly_credits']})",
            )
        ...
        if daily_usage >= self.limits["per_day"]:
            return (
                False,
                f"Daily limit reached ({daily_usage}/{self.limits['per_day']})",
            )
        ...
        if hourly_usage >= self.limits["per_hour"]:
            return (
                False,
                f"Hourly limit reached ({hourly_usage}/{self.limits['per_hour']})",
            )
        ...
        if minute_usage >= self.limits["per_minute"]:
            return (
                False,
                f"Per-minute limit reached ({minute_usage}/{self.limits['per_minute']})",
            )
    ...
    def get_usage_stats(self) -> Dict[str, Any]:
        ...
        return {
            "minute": {
                ...
                "remaining": max(
                    0, self.limits["per_minute"] - self._get_counter("minute")
                ),
            },
            "hour": {
                ...
                "remaining": max(
                    0, self.limits["per_hour"] - self._get_counter("hour")
                ),
            },
            "day": {
            ...
            "month": {
                ...
                "remaining": max(
                    0, self.limits["monthly_credits"] - self._get_counter("month")
                ),
            },
