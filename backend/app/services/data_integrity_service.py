...
class DataIntegrityService:
    ...
    def calculate_flight_checksum(self, flight_log: FlightLog, positions: List[FlightPosition]) -> str:
        ...
        flight_data = {
            ...
            'departure_time': flight_log.departure_time.isoformat() if flight_log.departure_time else None,
            'arrival_time': flight_log.arrival_time.isoformat() if flight_log.arrival_time else None,
            'positions': []
        }
    ...
    def verify_flight_integrity(self, db: Session, flight_id: str) -> Tuple[bool, Optional[str]]:
        """Verify a single flight's data integrity"""
        try:
            # Get flight log
            flight_log = db.query(FlightLog).filter(FlightLog.flight_id == flight_id).first()
            if not flight_log:
            ...
            for i in range(1, len(sorted_positions)):
                time_diff = (sorted_positions[i].timestamp - sorted_positions[i-1].timestamp).total_seconds()
                if time_diff > 60:  # More than 60 seconds gap
                    gaps.append(time_diff)

            # Check for impossible speeds (helicopter max ~180 knots)
            invalid_speeds = [p for p in positions if p.ground_speed_knots and p.ground_speed_knots > 200]

            # Check for impossible altitudes (Phoenix area, helicopter ceiling ~20,000 ft)
            invalid_altitudes = [p for p in positions if p.altitude_feet and (p.altitude_feet < -500 or p.altitude_feet > 20000)]
            ...
            issues = []
            if gaps:
                issues.append(f"Time gaps detected: {len(gaps)} gaps, largest: {max(gaps):.0f}s")
            if invalid_speeds:
    ...
    def verify_random_sample(self, db: Session, sample_size: int = 10) -> Dict[str, any]:
        ...
        for i in range(1, len(sorted_positions)):
            ...
            if time_diff > 0:
                ...
                if prev.latitude and prev.longitude and curr.latitude and curr.longitude:
                    ...
                    if speed > 0.05:
                        errors.append(f"Impossible speed between positions at {prev.timestamp} and {curr.timestamp}")
            ...
            if prev.altitude_feet and curr.altitude_feet and time_diff > 0:
                ...
                if altitude_change > max_change:
                    errors.append(f"Impossible altitude change at {curr.timestamp}: {altitude_change}ft in {time_diff}s")
    ...
    def generate_integrity_report(self, db: Session) -> Dict[str, any]:
        ...
        try:
            ...
            report['database_stats'] = {
                ...
                'flights_with_positions': db.query(func.count(func.distinct(FlightPosition.flight_log_id))).scalar()
            }
            ...
            recent_flights = db.query(FlightLog).filter(
                FlightLog.created_at >= datetime.now(timezone.utc) - timedelta(days=1)
            ).all()
