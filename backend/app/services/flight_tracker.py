...
class CompleteFlightTracker:
    ...
    def get_active_flights(self) -> List[Dict]:
        ...
        try:
            ...
            for heli in helicopters:
                ...
                logger.info(
                    f"Phoenix PD helicopter active: {heli.aircraft_registration} at {heli.latitude},{heli.longitude}"
                )
    ...
    def get_complete_flight_track(self, flight_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            if positions:
                ...
                logger.info(
                    f"Downloaded complete track for {flight_id}: {len(tracks)} positions"
                )
                return {
                    "flight_id": flight_id,
                    "tracks": tracks,
                    "metadata": {
                        "registration": positions[0].registration
                        if positions
                        else None,
                        "callsign": positions[0].callsign if positions else None,
                        "aircraft_type": positions[0].aircraft_type
                        if positions
                        else None,
                    },
                }
    ...
    def detect_flight_changes(self) -> Tuple[List[str], List[str]]:
        ...
        for flight in current_flights:
            ...
            is_phoenix_pd = (
                registration in PHOENIX_PD_REGISTRATIONS
                or callsign in PHOENIX_PD_REGISTRATIONS
            )
            ...
            if flight_id not in self.active_flights:
                ...
                logger.info(
                    f"Phoenix PD helicopter detected: {flight_id} ({registration or callsign})"
                )
            else:
                ...
                if prev_alt > 500 and current_alt < 100 and current_alt >= 0:
                    logger.info(
                        f"Detected landing by altitude: {flight_id} dropped from {prev_alt}ft to {current_alt}ft"
                    )
        ...
        for flight_id in list(self.active_flights.keys()):
            if flight_id not in current_ids:
                ...
                logger.info(
                    f"Flight disappeared (landed): {flight_id} ({self.active_flights[flight_id]['registration']})"
                )
            elif self.active_flights[flight_id].get("landing_detected"):
                # Landing was detected by altitude
                time_since_landing = (
                    datetime.now(timezone.utc)
                    - self.active_flights[flight_id]["last_seen"]
                )
                if (
                    time_since_landing.total_seconds() > 120
                ):  # Wait 2 minutes after landing detection
                    landed_flights.append(flight_id)
                    logger.info(
                        f"Flight confirmed landed: {flight_id} ({self.active_flights[flight_id]['registration']})"
                    )
        ...
        try:
            ...
            for i, track in enumerate(tracks):
                ...
                if i > 0:
                    ...
                    lat_diff = abs(
                        track.get("latitude", 0) - prev_track.get("latitude", 0)
                    )
                    lon_diff = abs(
                        track.get("longitude", 0) - prev_track.get("longitude", 0)
                    )
            ...
            logger.info(
                f"✓ Imported complete flight {flight_id}: {len(tracks)} positions, "
        ...
        try:
            for flight in current_flights:
                ...
                if not flight_info.get("flight_log_id"):
                    ...
                    existing_log = (
                        self.db.query(FlightLog)
                        .filter(FlightLog.flight_id == flight_log_id)
                        .first()
                    )
                ...
                position = FlightPosition(
                    ...
                    else self._find_or_create_aircraft(flight_info).id,
    ...
    def _find_or_create_aircraft(self, flight_info: Dict) -> Aircraft:
        ...
        if registration:
            aircraft = (
                ...
                .filter(Aircraft.registration == registration)
                .first()
            )
            if aircraft:
                return aircraft
        ...
        if hex_code:
            aircraft = (
                self.db.query(Aircraft).filter(Aircraft.icao_code == hex_code).first()
            )
            if aircraft:
                return aircraft
    ...
    def _analyze_complete_path(self, tracks: List[Dict], flight_info: Dict) -> Dict:
        ...
        for i, track in enumerate(tracks):
            ...
            if i > 0:
                prev = tracks[i - 1]
                movement = (
                    (lat - prev.get("lat", 0)) ** 2 + (lon - prev.get("lon", 0)) ** 2
                ) ** 0.5
                        ...
                        hover_locations.append(
                            {
                                ...
                                "duration_seconds": consecutive_hover_positions
                                * 10,  # Approximate
                    ...
                    low_altitude_segments.append(
                        {
                            ...
                            "min_altitude": min(
                                p.get("alt", 1000) for p in segment_positions
                            ),
                        }
                    )
                low_alt_start_idx = None
        ...
        if hover_ratio > 0.2:
            ...
            notes.append(
                f"Hovering {hover_ratio:.0%} of flight ({len(hover_locations)} locations)"
            )
            privacy_score = 5
            legal_notes.append(
                f"Extended hovering at {len(hover_locations)} locations indicates targeted surveillance"
            )
        ...
        if low_alt_ratio > 0.3:
            ...
            legal_notes.append(
                "Extended low-altitude operations violate reasonable expectation of privacy"
            )
        ...
        if len(areas_covered) < len(tracks) / 10:  # Visiting same areas repeatedly
            ...
            privacy_score = max(privacy_score, 4)
            legal_notes.append(
                "Circular flight pattern indicates focused surveillance of specific area"
            )
        ...
        return {
            ...
            "pattern_notes": f"COMPLETE TRACK ({total_positions} positions); "
            + "; ".join(notes),
        ...
        while True:
            try:
                ...
                for flight_id in landed_flights:
                    logger.info(
                        f"Processing complete track for landed flight {flight_id}"
                    )
                    self.process_landed_flight(flight_id)

                # Log status
                logger.info(
                    f"Active flights: {len(self.active_flights)}, "
