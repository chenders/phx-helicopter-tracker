...
class FR24SDKService:
    ...
    def __init__(self):
        ...
        self.phoenix_pd_aircraft = [
            "N622FB", "N623FB", "N624FB", "N625FB", "N626FB", "N627FB", "N628FB"
        ]
        # Phoenix area bounds
            ...
            try:
                ...
                for flight in flights:
                    ...
                    if reg and reg in self.phoenix_pd_aircraft:
                        ...
                        logger.info(f"✅ Found {reg} via area search - Alt: {tracking_data.altitude_feet} ft, Speed: {tracking_data.ground_speed_knots} kts")
                ...
                try:
                    ...
                    if results and 'live' in results and results['live']:
                        for live_flight in results['live']:
                            ...
                            try:
                                logger.info(f"Getting detailed data for {registration} (ID: {flight_id})...")
                                ...
                                if flight_details:
                                    ...
                                    tracking_data = LiveTrackingData(
                                        ...
                                        icao_code=flight_id,
                                        callsign=aircraft_data.get('registration', registration),
                                        timestamp=datetime.now(timezone.utc),
                                        latitude=lat,
                                        longitude=lon,
                                        altitude_feet=alt,
                                        ...
                                        is_circling=False,
                                        over_residential=False,
                                        privacy_concern=alt and alt < 1000 if alt else False
                                    )
                                    
                                    live_aircraft.append(tracking_data)
                                    logger.info(f"✅ Found {registration} via search - Alt: {alt} ft, Speed: {spd} kts")
                                else:
                                    ...
                                    live_aircraft.append(tracking_data)
                                    logger.info(f"✅ Found {registration} (basic data only)")
                                    
                            except Exception as e:
                                logger.debug(f"Could not get detailed data for {flight_id}: {e}")
                                ...
                                live_aircraft.append(tracking_data)
                                logger.info(f"✅ Found {registration} at {detail.get('lat')}, {detail.get('lon')} (no detailed data)")
