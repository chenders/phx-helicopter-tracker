...
class FlightPathTracker:
    ...
    def __init__(self, max_positions: int = 100, max_age_minutes: int = 30):
        ...
        self.flight_paths: Dict[str, deque] = defaultdict(lambda: deque(maxlen=max_positions))
        
    def add_position(self, aircraft_registration: str, lat: float, lon: float, 
                    altitude: float = None, timestamp: datetime = None) -> None:
        ...
        if self.flight_paths[aircraft_registration]:
            last_time, last_lat, last_lon, _ = self.flight_paths[aircraft_registration][-1]
            if (abs(last_lat - lat) < 0.0001 and 
                abs(last_lon - lon) < 0.0001 and 
                abs((timestamp - last_time).total_seconds()) < 1):
                return
