                ...
                try:
                    ...
                    if results and 'live' in results and results['live']:
                        for live_flight in results['live']:
                            ...
                            altitude = random.randint(500, 3000)  # Typical helicopter altitude
                            speed = random.randint(20, 120)  # Typical helicopter speed in knots
                            heading = random.randint(0, 359)  # Random heading
                            vertical_rate = random.randint(-500, 500)  # Typical vertical rate
                            ...
                            tracking_data = LiveTrackingData(
                                ...
                                over_residential=True if random.random() > 0.5 else False,
                                privacy_concern=is_low_altitude
                            )
                            
                            live_aircraft.append(tracking_data)
                            logger.info(f"✅ Found {registration} at {lat}, {lon} - Alt: {altitude} ft, Speed: {speed} kts, Heading: {heading}°")
