...
class AbnormalPattern(Base):
    ...
    pattern_type = Column(String(50), nullable=False, index=True)  # sky_writing, excessive_hovering, repetitive_circling, etc.
    confidence_score = Column(Float, nullable=False)  # 0.0 to 1.0
    
    # Detection details
    detection_metadata = Column(JSONB, nullable=False)  # Stores all metrics and analysis
    detected_at = Column(DateTime(timezone=True), server_default=func.now())
    
    # Human review
    reviewed = Column(String(10), default="pending", index=True)  # pending, confirmed, false_positive
