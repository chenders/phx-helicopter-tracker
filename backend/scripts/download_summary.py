...
def display_download_summary():
    ...
    try:
        ...
        print(
            f"Active Aircraft: {', '.join([a.registration for a in active_aircraft])}"
        )
        print("-" * 70)
        ...
        for aircraft in active_aircraft:
            ...
            date_range = (
                ...
                .filter(FlightDiscovery.registration == aircraft.registration)
                .first()
            )
        ...
        total_flight_logs = (
            session.query(FlightLog)
            .filter(FlightLog.aircraft_id.in_([a.id for a in active_aircraft]))
            .count()
        )
        ...
        if total_discoveries > 0:
            success_rate = (
                (total_downloaded / (total_downloaded + total_errors)) * 100
                if (total_downloaded + total_errors) > 0
                else 0
            )
        ...
        ) * 20  # 20 credits per track download
        credits_discoveries = (
