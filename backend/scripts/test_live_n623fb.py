...
async def test_live_tracking():
    ...
    try:
        ...
        async with fr24_api_service:
            ...
            if all_positions:
                ...
                for pos in all_positions:
                    print(
                        f"   - {pos.registration}: Alt {pos.altitude_feet} ft, Speed {pos.ground_speed_knots} kts"
                    )
            else:
        ...
        tracking_data = await unified_tracking_service.get_live_tracking_data(
            phoenix_pd_only=True, active_only=True, min_altitude=None, max_altitude=None
        )
        ...
        if tracking_data:
            ...
            for td in tracking_data:
                print(
                    f"   - {td.aircraft_registration}: {td.latitude:.4f}, {td.longitude:.4f} @ {td.altitude_feet} ft"
                )
        else:
