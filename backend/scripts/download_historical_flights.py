        ...
        try:
            ...
            for aircraft in active_aircraft:
                ...
                for chunk_start in range(0, days_back, 14):
                    ...
                    logger.info(f"  Fetching {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}...")
                ...
                for flight_data in all_flights:
                    try:
                        ...
                        existing = session.query(FlightDiscovery).filter(
                        ...
                        if flight_data.get('departure_time'):
                            ...
                            if isinstance(dep_str, str):
                                ...
                                departure_time = datetime.fromtimestamp(dep_str, tz=timezone.utc)

                        if flight_data.get('arrival_time'):
                            arr_str = flight_data['arrival_time']
                            if isinstance(arr_str, str):
                                arrival_time = datetime.fromisoformat(arr_str.replace('Z', '+00:00'))
                            else:
                                arrival_time = datetime.fromtimestamp(arr_str, tz=timezone.utc)
                        ...
                        logger.error(f"    Error processing flight {flight_data.get('fr24_id', 'unknown')}: {e}")
                        continue
            ...
            if total_new_flights > 0:
                ...
                for flight in flights_to_download:
                    try:
                        logger.info(f"  Downloading track for {flight.registration} flight {flight.fr24_id}...")
                        ...
                        if positions and len(positions) > 0:
                            ...
                            flight_log = FlightLog(
                                aircraft_id=next((a.id for a in active_aircraft if a.registration == flight.registration), None),
