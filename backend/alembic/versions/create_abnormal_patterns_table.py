...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "reviewed", sa.String(length=10), server_default="pending", nullable=True
        ),
    ...
    op.create_index(
        "idx_abnormal_patterns_flight_log_id", "abnormal_patterns", ["flight_log_id"]
    )
    op.create_index(
        "idx_abnormal_patterns_pattern_type", "abnormal_patterns", ["pattern_type"]
    )
    op.create_index("idx_abnormal_patterns_reviewed", "abnormal_patterns", ["reviewed"])
    op.create_index(
        "idx_abnormal_patterns_detected_at", "abnormal_patterns", ["detected_at"]
    )
