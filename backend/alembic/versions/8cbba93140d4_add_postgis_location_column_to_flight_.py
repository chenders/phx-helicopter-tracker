...
def upgrade() -> None:
    ...
    op.execute("CREATE INDEX IF NOT EXISTS idx_positions_location_gist ON flight_positions USING gist(location)")
...
def downgrade() -> None:
    # Drop the trigger
    op.execute("DROP TRIGGER IF EXISTS flight_position_location_trigger ON flight_positions")

    # Drop the function
    op.execute("DROP FUNCTION IF EXISTS update_flight_position_location()")

    # Recreate the btree index on latitude/longitude
    op.execute("CREATE INDEX idx_positions_location ON flight_positions(latitude, longitude)")
