...
async def init_db(database_url: str = None, drop_all: bool = False):
    ...
    if database_url is None:
        database_url = (
            "postgresql+asyncpg://postgres:postgres@localhost:5433/phoenix_helicopters"
        )
    ...
    async with engine.begin() as conn:
        ...
        print(
            f"✓ Database tables created successfully for: {database_url.split('/')[-1]}"
        )
...
async def init_test_db(drop_all: bool = True):
    ...
    """
    test_db_url = (
        "postgresql+asyncpg://postgres:postgres@localhost:5433/phoenix_helicopters_test"
    )
